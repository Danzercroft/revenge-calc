...
class ExchangeService:
    ...
    async def fetch_current_candle(self, exchange: ccxt.Exchange, symbol: str, timeframe: str) -> Optional[Dict]:
        """Получает текущую свечу с биржи"""
        try:
            # Получаем последние 2 свечи (текущую и предыдущую)
            loop = asyncio.get_event_loop()
            ohlcv = await loop.run_in_executor(None, exchange.fetch_ohlcv, symbol, timeframe, None, 2)
            if ohlcv:
    ...
    async def fetch_historical_candles(self, exchange: ccxt.Exchange, symbol: str, timeframe: str, 
                                since: datetime, limit: int = 1000) -> List[Dict]:
        """Получает исторические свечи с биржи"""
        try:
            since_timestamp = int(since.timestamp() * 1000)
            loop = asyncio.get_event_loop()
            ohlcv = await loop.run_in_executor(None, exchange.fetch_ohlcv, symbol, timeframe, since_timestamp, limit)
    ...
    async def get_missing_candles_timerange(self, db: Session, symbol: str, exchange_id: int, 
